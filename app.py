...
st.caption(
    "AI-powered OCR system with field-level validation and audit logic for MRN images."
)
...
def validate_feeder_name(value: str) -> str:
    if not value:
        return ""
...
def validate_address(value: str) -> str:
    ...
    if len(value) < 8:
        return ""
    if re.search(r"[A-Z]{2,}\s*\d{3,}", value):
        return ""
    return value

KNOWN_METER_MAKES = [
    "HPL", "L&T", "LT", "SECURE", "GENUS",
    "SCHNEIDER", "ELSTER", "LANDIS",
    "CROMPTON", "IMP", "VISIONTEK"
]

def validate_meter_make(value: str) -> str:
    if not value:
        return ""
    value = value.upper().strip()
    for make in KNOWN_METER_MAKES:
        if make in value:
            return make
    return ""

# =====================================================
# FEEDER RECOVERY USING CONTEXT (KEY FIX)
# =====================================================
def recover_feeder_from_context(raw_text: str) -> str:
    lines = raw_text.splitlines()

    for i, line in enumerate(lines):
        if re.search(r"feeder|fdr", line, re.IGNORECASE):
            candidate = re.sub(
                r"feeder|fdr|[:\-]", "", line, flags=re.IGNORECASE
            ).strip()

            if validate_feeder_name(candidate):
                return candidate
...
"""
    ...
    validated = {
        ...
        "feeder_name": validate_feeder_name(data.get("feeder_name")),
        "consumer_name": data.get("consumer_name", "").strip(),
        "consumer_address": validate_address(data.get("consumer_address")),
        ...
        "old_meter_make": validate_meter_make(data.get("old_meter_make")),
        "new_meter_no": validate_meter_no(data.get("new_meter_no")),
        "new_meter_reading": validate_reading(data.get("new_meter_reading")),
        "new_meter_make": validate_meter_make(data.get("new_meter_make")),
        "consumer_category": data.get("consumer_category", "").strip()
    }

    # 🔥 FEEDER FIX APPLIED HERE
    if not validated["feeder_name"]:
        validated["feeder_name"] = recover_feeder_from_context(raw_text)
...
if uploaded_file:
    ...
    if st.button("🔍 Extract & Audit Data"):
        with st.spinner("Extracting, validating, and auditing MRN data..."):
            output = extract_mrn_data(temp_path)

        st.success("✅ Extraction & Audit Complete")
        st.code(json.dumps(output, indent=2), language="json")

        st.download_button(
            "⬇️ Download JSON",
            data=json.dumps(output, indent=2),
            file_name="mrn_audited_data.json",
